...
def fetchWordData(inputWord):
    ...
    pos = map(lambda x: [z.text for z in x.select('em')][0], posTags)
    meaning = map(lambda x: [z.text for z in x.select('strong')][0], posTags)

    for defnNum in xrange(0, definitionCount):
    ...
    clean = lambda x: x.strip().replace(u'\u201d', '"').replace(u'\u201c', '"')
...
class Word:
    ...
    def filter(self, defnNum='all', **filters):
        ...
        if not made_changes:
            ...
            for x in xrange(0, len(self.data)):
                ...
                if (partOfSpeech == None) or (self.data[x]['partOfSpeech'] == partOfSpeech):
                    fdata.append({
                        'syn': filter(lambda y: f(y), self.data[x]['syn']),
                        'ant': filter(lambda y: f(y), self.data[x]['ant'])
                    })
                else:
                    continue
        ...
        for x in xrange(startRange, endRange):
            ...
            for entry_type in ['syn', 'ant']:
                ...
                for y in xrange(0, len(c_def[entry_type])):
    ...
    def synonyms(self,defnNum=0,allowEmpty=True,**filters):
        data = map(lambda x: x['syn'] if 'syn' in x else [],
            self.filter(defnNum=defnNum, filters=filters))

        data = map(lambda x: map(lambda y: y[0], x), data)
        ...
        if defnNum != 'all':
            ...
            if allowEmpty == True:
                ...
                return filter(lambda x: len(x) is not 0, data)

    def antonyms(self,defnNum=0,allowEmpty=True,**filters):
        data = map(lambda x: x['ant'] if 'ant' in x else [],
            self.filter(defnNum=defnNum, filters=filters))
